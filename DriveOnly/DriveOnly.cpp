    ...
    switch (self.autonState) {
      case AS_GoUp: {
        ...
        if (absd(self.HeightEnc.GetDistance() - self.HeightSetpoint) < 2.0)
          self.autonState = AS_DriveForward;
      }
      break;
      case AS_DriveForward: {
        ...
        if (self.targetArea != 0 ) {
          ...
          float speed = 0;
          if (self.ultraho.GetDistance() > 2.75) {
          ...
          } else if (self.ultraho.GetDistance() <= 2.75) {
            speed = 0;
            self.autonState = AS_GetHigh;
          }
